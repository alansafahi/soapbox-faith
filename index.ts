...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        consent_token: { type: "string", description: "Opaque SoapBox consent token with the 'prayer:write' scope, generated by the user in the SoapBox app (Settings › AI Agents). The first agent key to use a token is bound to it; a token no agent uses within 24 hours of being created stops working. An invalid, expired, or wrong-scope token returns an authorization error.", minLength: 16, examples: ["sbxc_9f3a2b7c4d5e6f7a…"] },
  ...
  },
  {
    ...
    description:
      ...
      "double-charging on retries. Amounts are in the smallest currency unit (cents). This moves the user's " +
      "money: confirm the church and amount with the user before every call. The user is notified of every gift.",
    inputSchema: {
      type: "object",
      properties: {
        consent_token: { type: "string", description: "Opaque SoapBox consent token with the 'giving:write' scope, carrying the user's pre-authorized per-gift and monthly caps. Generated by the user in the SoapBox app (Settings › AI Agents). The first agent key to use a token is bound to it; a token no agent uses within 24 hours of being created stops working.", minLength: 16, examples: ["sbxc_9f3a2b7c4d5e6f7a…"] },
        ...
        amount_cents: { type: "integer", description: "Gift amount in the smallest currency unit (cents), at least 100. Must be within the user's per-gift and monthly caps or the call is rejected.", minimum: 100, examples: [2500] },
        currency: { type: "string", description: "Optional ISO 4217 currency code (lowercase). Must be the church's own currency; omit it to use that. AI-agent gifts are available in USD, CAD, GBP, EUR, AUD and NZD.", pattern: "^[a-z]{3}$", examples: ["usd", "cad"] },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        consent_token: { type: "string", description: "Opaque SoapBox consent token with the 'context:read' scope. The user generates it in the SoapBox app (Settings › AI Agents). The first agent key to use a token is bound to it; a token no agent uses within 24 hours of being created stops working. Returns only the user's declared profile (denomination, preferred language, faith journey, ministry interests) — never their private prayers or journal. An invalid, expired, or wrong-scope token returns an authorization error.", minLength: 16, examples: ["sbxc_9f3a2b7c4d5e6f7a…"] },
      },
